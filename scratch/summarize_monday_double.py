...
csv_path = r'c:\Users\Dimitar\RunningCoach\completed\w02\2026-09-07-i184302852_intervals.csv'
...
def summarize_slice(sub_rows, title):
    tot_d = 0
    tot_mov_t = 0
    tot_elap_t = 0
    tot_gain = 0
    ...
    print(f"Zones: Rec (<142): {time_rec/tot_mov_t*100:.1f}% | GA (142-152): {time_ga/tot_mov_t*100:.1f}% | MP (153-165): {time_mp/tot_mov_t*100:.1f}% | LT (>165): {time_lt/tot_mov_t*100:.1f}%")
    print()

# Part 1: Morning commute (Intervals 1-13, 0 - 6.5 km)
# Part 2: Afternoon commute (Intervals 14-28, 6.5 - 13.62 km)
# Or if interval 14 is end of morning run, let's see. Notice interval 14 has elapsed time 20535s.
# That means interval 14 was either running into work or running out from work.
# Let's check both or split at interval 14.
summarize_slice(rows[:13], "Part 1: Morning Commute (0 - 6.50 km)")
summarize_slice(rows[13:], "Part 2: Afternoon Commute (6.50 - 13.62 km, 7.12 km)")
summarize_slice(rows, "Combined Full Day (13.62 km)")
